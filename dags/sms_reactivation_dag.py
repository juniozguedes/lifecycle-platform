...
with DAG(
    ...
    def validate_audience(audience_data: dict[str, Any]) -> dict[str, Any]:
        ...
        is_valid, error_msg = validate_audience_anomalies(recipients, count, ANOMALY_THRESHOLD_MULTIPLIER, historical_avg)
        if not is_valid:
            return {"is_valid": False, "error_message": error_msg, "audience_count": count, "audience": []}
    ...
    def execute_campaign_send_task(validation_data: dict[str, Any]) -> dict[str, Any]:
        ...
        if not is_valid:
            ...
            return {"total_sent": 0, "total_failed": 0, "total_skipped": 0, "elapsed_seconds": 0.0, "status": "skipped", "error_message": error_msg, "audience_count": 0}
        recipients = validation_data.get("audience", [])
        if not recipients:
            return {"total_sent": 0, "total_failed": 0, "total_skipped": 0, "elapsed_seconds": 0.0, "status": "skipped", "error_message": "empty_audience", "audience_count": 0}
    ...
    def log_results_and_notify(campaign_data: dict[str, Any]) -> dict[str, bool]:
        ...
        if is_success:
            ...
            notification_sent = send_slack_alert(dag_id=DAG_ID, task_id=None, error_message=campaign_data.get("error_message", "Unknown error"), execution_date=exec_date)
